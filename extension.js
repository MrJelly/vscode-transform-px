...
function activate(context) {
  ...
  var disposable = vscode.commands.registerTextEditorCommand(
    ...
    function(textEditor, textEditorEdit) {
      const config = vscode.workspace.getConfiguration("px-to-PX");
      const pxPerPX = config.get("px-per-PX");
      var regexStr = "([0-9]*\\.?[0-9]+)PX";
      placeholder(
        regexStr,
        (match, value) => `${PX2px(value, pxPerPX)}px`,
        textEditor,
        textEditorEdit
      );
    }
  ...
  disposable = vscode.commands.registerTextEditorCommand(
    "extension.pxToPXAndPXTopx",
    function(textEditor, textEditorEdit) {
      const config = vscode.workspace.getConfiguration("px-to-PX");
      console.log('=====',config)
      const pxPerPX = config.get("px-per-PX");
      var regexStr = "([0-9]*\\.?[0-9]+)(px|PX)";
      placeholder(
        regexStr,
        (match, value, unit) =>
          unit == "px"
            ? `${px2PX(value, pxPerPX)}PX`
            : `${PX2px(value, pxPerPX)}px`,
        textEditor,
        textEditorEdit
      );
  ...
  disposable = vscode.commands.registerCommand(
    ...
    async () => {
      ...
      var inputValue = await vscode.window.showInputBox({
        prompt: "Number of pixels per 1 PX. Default is 16.",
        value: `${pxPerPX}`,
      ...
      const newPxPerPX = parseInt(inputValue);
      if (isNaN(newPxPerPX)) {
        vscode.window.showErrorMessage(
          `${inputValue} is not a valid integer to set as px per PX.`
        );
        return;
      }
      config.update("px-per-PX", newPxPerPX);
      vscode.window.showInformationMessage(
        `Px per PX has updated to ${newPxPerPX}px`
      );
    }
  );
  context.subscriptions.push(disposable);
...
function px2PX(px, pxPerPX) {
  ...
  const value = parseFloat((px / pxPerPX).toFixed(maxDecimals));
  return value;
}
function PX2px(PX, pxPerPX) {
  const config = vscode.workspace.getConfiguration("px-to-PX");
